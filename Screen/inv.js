...
export default function Project({ navigation }) {
 ...
 const urlInv='https://dsms0-7e9f.restdb.io/rest/invites';
    ...
    async function getprojects() {
      let response = await fetch(urlInv, options);
  ...
  return (
    ...
    shadowOpacity: 1,}}
          onChangeText={(searching) => {
            const url =
              'https://dsms0-7e9f.restdb.io/rest/invites?q={ "_createdby": {"$regex" :"' +
              searching +
              '"}}';
            ...
            function allgetdata() {
              fetch(
                'https://dsms0-7e9f.restdb.io/rest/invites',
                options
              )
                .then((res) => res.json())
                .then((resjson) => {
          ...
          placeholder="Search For a invites . ."></TextInput>
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text style={styles.projectsinfo}>
                Created by : {item._createdby} 
             </Text>
              <Text style={styles.projectsinfo}>
                Project Version : {item._version}
              </Text>
              ...
              <TouchableOpacity style={styles.flatlistusersR}      onPress={() => {
                ...
                async function del() {
                  ...
                  if (req.ok) {
                    alert('The invites ' + item._createdby + ' Deleted');
                  }
                }
              }}>
